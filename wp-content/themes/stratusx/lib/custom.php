		...
		if ( ! empty( $social_icons ) ) {
			foreach( $social_icons as $social_icon ) {
				if (isset($social_icon["themo_social_url"]) && $social_icon["themo_social_url"] >""){
				    ...
				    $output .= "<a ".$link_target_att." href='".$social_icon["themo_social_url"]."' aria-label='Social'><i class='".$social_icon["themo_social_font_icon"]."'></i></a>";
                }else{
		...
		if ( function_exists( 'get_theme_mod' ) ) {
			...
			if (isset($icon_block) && is_array($icon_block)  && !empty($icon_block)) {
				...
				foreach( $icon_block as $icon ) {
					...
					$output .= $href_open."<i class='".esc_attr($icon["themo_contact_icon"])."'></i> ".wp_kses_post($icon["title"]) .$href_close;
					$output .= '</div>';
				}
				$output .= "</div>";
			}
...
function themo_return_footer_logo() {
    ...
    if ( function_exists( 'get_theme_mod' ) ) {
        ...
        if(isset($footer_logo_src) && $footer_logo_src > ""){
            $output .= $href_open . "<img src='".esc_url($footer_logo_src)."' " .$footer_logo_width ." ". $footer_logo_height. " alt='Healthray Logo'>" . $href_close;
        }
