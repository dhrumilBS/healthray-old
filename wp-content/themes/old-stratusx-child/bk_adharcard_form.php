    ...
    if (isset($_POST['otp']) && isset($_POST['transaction_id']) && isset($_POST['otpMobileno'])) {
        ...
        if (!is_wp_error($response)) {
            ...
            if (isset($body['success']) && $body['success'] === true) {
                wp_send_json_success(['message' => $body['message'] ?? 'OTP verified successfully.']);
            } else {
...
// Handle form submission via AJAX
function mobile_form_submit_ajax()
{
    $aadhaar_number = sanitize_text_field($_POST['aadhaar']);
...
add_action('wp_ajax_mobile_form_submit', 'mobile_form_submit_ajax');
add_action('wp_ajax_nopriv_mobile_form_submit', 'mobile_form_submit_ajax');
