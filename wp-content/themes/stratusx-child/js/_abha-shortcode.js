jQuery(document).ready(($) => {
	...
	function populatePhrDropdown(mappedPhrAddresses, transactionId) {
		...
		$('#select-phr-btn').off('click').on('click', async function () {
			...
			const transactionId = $transactionIdField.val().trim(); // Replace with the correct field ID

			await handlePHRLogin(transactionId, selectedAddress, true);
		});
	}
	...
	$resendButton.on('click', async (e) => {
		...
		console.log({ TransactionID: transactionId, value: ADHARNUMBER, type: type });
	...
	async function handlePHRLogin(transactionId, phrAddress, already_registered = true) {
		if (!phrAddress) {
			return showError('PHR Address is required.');
		}
		...
		try {
			...
			if (loginResponse.success) {
				...
				if (imageUrl) {
					...
					showSuccess('Login successful, but no image was returned.');
				}
			} else {
	...
	async function fetchSuggestion(transaction_id) {
		try {
			const response = await $.get(ajax_obj, { action: "get_PHR_suggestion", transaction_id: transaction_id });
			...
			if (response.success) {
				...
				suggestions.forEach((name) => {
					...
					$suggestionItem.on('click', function () {
						...
						console.log("Selected ADDRESS: ", $(this).data('address'));
					});
				});
			} else {
				console.log(`Error fetching states: ${response.data.message}`);
	...
	$('#getSuggestion').on('click', async function (e) {

		fetchSuggestion($transactionIdField.val().trim())

	});
	$('#submitSuggestion').on('click', (e) => {
		e.preventDefault();
		const item = $('.suggestion-list').find('.selected-address');
		const PHRaddress = item.data('address');
		console.log("getSuggestion", PHRaddress);
	})
	$('#submitSuggestion').off('click').on('click', async function () {
		const transactionId = $transactionIdField.val().trim();
		const phrAddress = $('.suggestion-list').find('.selected-address').data('address');

		if (!phrAddress) {
			return showError('Please select a PHR Address before submitting.');
		}
