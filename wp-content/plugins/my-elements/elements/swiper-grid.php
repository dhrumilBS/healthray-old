...
class Ml_Swiper_Widget extends Widget_Base
{
	...
	public function get_style_depends() { return ['owl.carousal']; }
...
?>
		...
		<div class="global-client-logo-slider owl-theme owl-carousel" <?= $this->get_render_attribute_string('slider'); ?>>
			<?php
			$html = '';
			foreach ($settings['gallery'] as $i => $image) {

				$html .= '<div class="img-slide">';
				$html .=  wp_get_attachment_image($image['id'], 'full', false, ['class' => 'image']);
				...
				if ($settings['caption'] == 'yes') {
					...
					$html .= "<p class='image-title text-center'>" .  $caption . "</p>";
				}
